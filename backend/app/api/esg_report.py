...
from app.services.esg_report_service import generate_brsr_principle1, generate_brsr_principle4, generate_brsr_principle6, generate_brsr_principle7, generate_brsr_principle2, generate_brsr_principle5, generate_brsr_principle9, generate_brsr_principle8, generate_gri_305, generate_esrs_e1, generate_trend
...
@router.get("/brsr-principle7")
def brsr_principle7(
    year: int = 2024,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """BRSR Principle 7 (Public and Regulatory Policy Advocacy) report for the caller's organization."""
    return generate_brsr_principle7(
        db=db,
        organization_id=current_user.organization_id,
        reporting_year=year,
    )
...
@router.get("/brsr-principle4")
def brsr_principle4(
    year: int = 2024,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """BRSR Principle 4 (Stakeholder Responsiveness) report for the caller's organization."""
    return generate_brsr_principle4(
        db=db,
        organization_id=current_user.organization_id,
        reporting_year=year,
    )
...
@router.get("/brsr-principle2")
def brsr_principle2(
    year: int = 2024,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """BRSR Principle 2 (Sustainable and Safe Goods and Services) report for the caller's organization."""
    return generate_brsr_principle2(
        db=db,
        organization_id=current_user.organization_id,
        reporting_year=year,
    )
...
@router.get("/brsr-principle5")
def brsr_principle5(
    year: int = 2024,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """BRSR Principle 5 (Human Rights) report for the caller's organization."""
    return generate_brsr_principle5(
        db=db,
        organization_id=current_user.organization_id,
        reporting_year=year,
    )
...
@router.get("/brsr-principle9")
def brsr_principle9(
    year: int = 2024,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """BRSR Principle 9 (Responsible Engagement with Consumers) report for the caller's organization."""
    return generate_brsr_principle9(
        db=db,
        organization_id=current_user.organization_id,
        reporting_year=year,
    )
